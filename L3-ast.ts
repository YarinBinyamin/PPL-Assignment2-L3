...
// L31
export type ClassExp = {tag: "ClassExp"; fields: VarDecl[], methods: Binding[]; env: Env; }
...
//     }
//     const fieldsMap = map((x: Sexp) => x.toString(), fields);
//     const fieldVars = map(makeVarDecl, fieldsMap);
//     const methodsResult = mapResult(parseBinding, methods);
//     return bind(methodsResult, (methods: Binding[]) => makeOk(makeClassExp(fieldVars, methods)));
// }
...
//     if (!isNonEmptyList(binding)) {
//         return makeFailure('Malformed binding in "class" expression');
//     }
//     const [method, body] = binding;
//     if (!isIdentifier(method)) {
//         return makeFailure('Malformed method in "class" expression');
//     }
//     return bind(parseL3CExp(body), (body: CExp) => makeOk(makeBinding(method.toString(), body)));
// }

/////////////////////////////////
//L31
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    if (!isGoodFields(fields)) { // check if fields is a list of strings
        return makeFailure('Malformed fields in "class" expression');
    }
    if (!isGoodMethods(methods)) { // check if methods is a list of lists of length 2
        return makeFailure('Malformed methods in "class" expression');
    }
    // converts fields to VarDecl[] object
    //console.log(fields);
    const fieldVars = map((field: Sexp) => makeVarDecl(field.toString()), fields);
    //console.log(fieldVars);
    // converts methods to Binding[] object
    const methodResults = mapResult(parseBinding, methods);
    return bind(methodResults, (methodBindings: Binding[]) => makeOk(makeClassExp(fieldVars, methodBindings))); // first check method
};

const isGoodFields = (fields: Sexp): fields is Sexp[] =>
    isArray(fields) && allT(isIdentifier, fields);

const isGoodMethods = (methods: Sexp): methods is Sexp[] =>
    isArray(methods) && allT(isNonEmptyList<Sexp>, methods) && methods.every(m => m.length === 2);

const parseBinding = (binding: Sexp): Result<Binding> => {
    if (!isNonEmptyList(binding) || binding.length !== 2) { //each binding is a list of length 2
        return makeFailure("Malformed binding in class expression");
    }
    const [method, body] = binding; // method is the first element of the binding, body is the second
    if (!isIdentifier(method)) {
        return makeFailure("Malformed method name in class expression:");
    }
    return bind(parseL3CExp(body), (parsedBody: CExp) => makeOk(makeBinding(method.toString(), parsedBody)));
};
