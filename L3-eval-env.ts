...
import { isBoolExp, isCExp, isLitExp, isNumExp, isPrimOp, isStrExp, isVarRef,
         ...
         parseL3Exp,  DefineExp, ClassExp, makeClassExp, isClassExp} from "./L3-ast";
import { applyEnv, makeEmptyEnv, makeExtEnv, Env } from "./L3-env-env";
import { isClosure, makeClosureEnv, Closure, Value, isClass, makeClassEnv , Class, makeObject, isObject, Object, isSymbolSExp} from "./L3-value";
import { applyPrimitive } from "./evalPrimitive";
import { allT, first, rest, isEmpty, isNonEmptyList, cons } from "../shared/list";
...
const applyClassEnv = (proc: Class, args: Value[]): Result<Value> =>
    makeOk(makeObject(proc, args, proc.env));   
...
const applyObject = (proc: Object, args: Value[]): Result<Value> => {
    if(isNonEmptyList<Value>(args)){
        if(isSymbolSExp(args[0])){
            ...
            const clos = makeClosureEnv(inside, methodClosure.body, proc.env);
                if (isClosure(clos)) {
                    return applyClosure(clos, arrays);
                }
                else {
